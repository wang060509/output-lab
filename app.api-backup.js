...
const AI_CONFIG_STORAGE_KEY = "output-lab-ai-config";
const OPENAI_API_URL = "https://api.openai.com/v1/chat/completions";
const OPENAI_MODEL = "gpt-4o-mini";
const OPENROUTER_API_URL = "https://openrouter.ai/api/v1/chat/completions";
const OPENROUTER_MODEL = "openrouter/free";
let liveAIStatusMessage = "";
...
function persistAIConfig() {
  ...
  try {
    const normalized = normalizeAIConfig(window.AI_CONFIG);
    window.AI_CONFIG = normalized;
    storage.setItem(AI_CONFIG_STORAGE_KEY, JSON.stringify({
      apiKey: typeof normalized.apiKey === "string" ? normalized.apiKey.trim() : "",
      apiUrl: normalized.apiUrl,
      model: normalized.model
    }));
  } catch (error) {
    // Ignore storage failures to keep demo mode stable.
  }
}
...
function normalizeAIConfig(config) {
  ...
  if (nextConfig.apiKey.startsWith("sk-or-")) {
    nextConfig.apiUrl = OPENROUTER_API_URL;
    if (!nextConfig.model || nextConfig.model === "gpt-4o-mini" || nextConfig.model === OPENAI_MODEL) {
      nextConfig.model = OPENROUTER_MODEL;
    }
    return nextConfig;
  }

  if (nextConfig.apiKey.startsWith("sk-proj-") || (nextConfig.apiKey.startsWith("sk-") && !nextConfig.apiKey.startsWith("sk-or-"))) {
    nextConfig.apiUrl = OPENAI_API_URL;
    nextConfig.model = OPENAI_MODEL;
    return nextConfig;
  }
...
function getAIProvider(config = window.AI_CONFIG || {}) {
  const apiKey = typeof config.apiKey === "string" ? config.apiKey.trim() : "";
  const apiUrl = typeof config.apiUrl === "string" ? config.apiUrl.trim() : "";

  if (apiKey.startsWith("sk-or-") || apiUrl.includes("openrouter.ai")) {
    return "OpenRouter";
  }
...
function logAIConfig() {
  console.log("[Output Lab] API config", {
    provider: getAIProvider(window.AI_CONFIG),
    apiUrl: window.AI_CONFIG.apiUrl,
    model: window.AI_CONFIG.model,
    apiKey: maskApiKey(window.AI_CONFIG.apiKey)
  });
}

function showLocalhostHint() {
  if (window.location.protocol === "file:") {
    alert("\u8bf7\u4f7f\u7528 http://localhost:5500 \u6253\u5f00\u9879\u76ee");
  }
}

function legacyRuntimeHintUnused() {
  if (window.location.protocol === "file:") {
    alert("请使用 http://localhost:5500 打开项目");
  }
}

function ensureRuntimeHints() {
  showLocalhostHint();
}

function setLiveAIStatus(message) {
  liveAIStatusMessage = message || "";
  const statusNode = document.getElementById("live-ai-status");
  if (!statusNode) {
    return;
  }

  statusNode.textContent = liveAIStatusMessage;
  statusNode.style.display = liveAIStatusMessage ? "block" : "none";
}

window.AI_CONFIG = normalizeAIConfig({
  ...window.AI_CONFIG,
  ...readStoredAIConfig()
});
persistAIConfig();
...
function renderTopBar() {
  ...
  const isDemoMode = window.APP_MODE === "demo";
  const isLiveMode = window.APP_MODE === "live";
  const shouldShowFileHint = window.location.protocol === "file:";
  const provider = getAIProvider(window.AI_CONFIG);
  topBar.innerHTML = `
    <div class="brand">
    ...
    <div>
      <div class="mode-toggle" style="display:flex; gap:8px; align-items:center;">
        ...
        <button id="mode-demo" class="mode-btn ${isDemoMode ? "active" : ""}">Demo</button>
        <button id="mode-live" class="mode-btn ${isLiveMode ? "active" : ""}">Live AI</button>
      </div>
      <div id="api-config" style="display:${isLiveMode ? "block" : "none"}; margin-top:8px;">
        ${shouldShowFileHint ? '<div style="margin-bottom:8px; color:#ff6b6b; font-size:12px; line-height:1.5;">Please run this project from http://localhost:5500 instead of opening index.html directly.</div>' : ""}
        <div id="live-ai-status" style="display:${liveAIStatusMessage ? "block" : "none"}; margin-bottom:8px; color:#ff6b6b; font-size:12px; line-height:1.5;">${escapeHtml(liveAIStatusMessage)}</div>
        <div style="margin-bottom:8px; color:var(--muted); font-size:12px;">Provider: ${escapeHtml(provider)}</div>
        <input
          id="api-key-input"
          type="password"
          placeholder="Enter API Key..."
          style="padding:4px 8px; font-size:12px; border:1px solid var(--line-2); background:transparent; color:var(--ink);"
        />
        <button id="save-api-key" style="margin-left:6px; font-size:12px;">
          Save
        </button>
      </div>
    </div>
  ...
  if (window.AI_CONFIG.apiKey) {
    const apiInput = document.getElementById("api-key-input");
    if (apiInput) {
      apiInput.value = window.AI_CONFIG.apiKey;
    }
  }

  setLiveAIStatus(liveAIStatusMessage);
}
...
function bindActions() {
  setTimeout(() => {
    ...
    if (saveBtn && apiInput) {
      saveBtn.onclick = () => {
        window.AI_CONFIG.apiKey = apiInput.value.trim();
        window.AI_CONFIG = normalizeAIConfig(window.AI_CONFIG);
        persistAIConfig();
        logAIConfig();
        setLiveAIStatus("");

        if (window.AI_CONFIG.apiKey) {
          alert("API key saved. Live AI enabled.");
        } else {
...
function safeJsonParse(content) {
  ...
  if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
    const objectSlice = content.slice(firstBrace, lastBrace + 1);
    try {
...
function normalizeAIScores(scores) {
  if (!scores || typeof scores !== "object") {
    return null;
  }

  const normalized = {
    clarity: clampScore(Number(scores.clarity)),
    engagement: clampScore(Number(scores.engagement)),
    structure: clampScore(Number(scores.structure)),
    credibility: clampScore(Number(scores.credibility)),
    visual: clampScore(Number(scores.visual)),
    cta: clampScore(Number(scores.cta)),
    audience: clampScore(Number(scores.audience))
  };

  const isValid = Object.values(normalized).every((value) => Number.isFinite(value) && value >= 1 && value <= 10);
  return isValid ? normalized : null;
}

function buildDiagnosePrompt(input, outputType) {
  return [
    `Output type: ${outputType || "LinkedIn Carousel"}`,
    "Analyze the content below for practitioner-facing communication quality.",
    ...
    '  "insight": "Explain why the content performs well or poorly.",',
    '  "repair": "Give concise repair strategy."',
    "}",
    "",
    "Content:",
    input
...
function buildRepairPrompt(input, outputType) {
  return [
    `Rewrite the content below as a ${outputType || "LinkedIn Carousel"}.`,
    "Return strict JSON only using this schema:",
    "{",
    '  "repairedText": "The fully rewritten output here.",',
    '  "changesMade": ["change 1", "change 2", "change 3"]',
    "}",
    "",
    "Preserve research fidelity, keep uncertainty where needed, and do not introduce unsupported evidence.",
    "",
    "Content:",
    input
  ].join("\n");
...
async function callAI({ task, input, outputType }) {
  const config = window.AI_CONFIG || {};
  const apiKey = config.apiKey;
  const apiUrl = config.apiUrl || OPENAI_API_URL;
  const model = config.model || OPENAI_MODEL;
  const provider = getAIProvider(config);

  if (!apiKey || !task || !input) {
    return null;
  }
  ...
  const systemPrompt = [
    "You are an Academic-to-Policy Communication Auditor.",
    "Preserve research fidelity.",
    "Do not fabricate evidence.",
    "Do not turn correlation into causation.",
    "Evaluate audience fit and actionability.",
    "Output JSON only."
  ].join(" ");

  const userPrompt = task === "repair"
    ? buildRepairPrompt(input, outputType)
    : buildDiagnosePrompt(input, outputType);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[Output Lab] AI request failed", {
      ...
      if (response.status === 403 && errorText.includes("not available in your region")) {
        setLiveAIStatus("The selected OpenRouter model is not available in your region. Please switch to another model.");
        return null;
      }
    ...
    console.log("[Output Lab] AI raw content", content?.slice(0, 1000));
    const parsed = safeJsonParse(content);

    if (!parsed) {
      console.warn("[Output Lab] Failed to parse AI JSON", content?.slice(0, 1000));
      return null;
    }

    if (task === "diagnose") {
      const scores = normalizeAIScores(parsed.scores);
      const issues = Array.isArray(parsed.issues)
        ? parsed.issues.filter((item) => typeof item === "string" && item.trim()).map((item) => item.trim())
        : [];

      if (!scores || typeof parsed.insight !== "string" || typeof parsed.repair !== "string") {
        return null;
      }
      ...
      return {
        ...
        repair: parsed.repair.trim()
      };
    }

    if (typeof parsed.repairedText !== "string") {
      return null;
    }

    return {
      repairedText: parsed.repairedText.trim(),
      changesMade: Array.isArray(parsed.changesMade)
        ? parsed.changesMade.filter((item) => typeof item === "string" && item.trim()).map((item) => item.trim())
        : []
    };
  } catch (error) {
    console.error("[Output Lab] AI request error", error);
    return null;
  } finally {
...
async function renderDiagnoseResults(scoreRows, insightText, repairText, scoreMap) {
  ...
  if (liveScore) {
    liveScore.textContent = `Live Score -> A:${scoreMap.audience} | Ac:${scoreMap.cta} | S:${scoreMap.structure} | M:${scoreMap.credibility}`;
  }
...
async function runDiagnose() {
  ...
  const isLiveMode =
    window.APP_MODE === "live" &&
    Boolean(window.AI_CONFIG && window.AI_CONFIG.apiKey);
  const isTryYourOwnMode = document.querySelector(".panel.active")?.id === "try-your-own";

  if (window.APP_MODE === "live" && !(window.AI_CONFIG && window.AI_CONFIG.apiKey)) {
    alert("Live AI mode requires API key. Falling back to demo mode.");
  }
  ...
  if (aiWhyLowScore) {
    aiWhyLowScore.textContent = "Generating live AI analysis...";
  }
  ...
  try {
    ...
    const aiDiagnose = isLiveMode
      ? await callAI({
        task: "diagnose",
        input,
        outputType: caseData.topBar.fields.find((field) => field.label === "Type")?.value || "LinkedIn Carousel"
      })
      : null;

    const scoreMap = aiDiagnose?.scores || localScoreMap;
    const scoreRows = [
    ...
    latestDiagnoseState = {
      input,
      issues: [...(aiDiagnose?.issues?.length ? aiDiagnose.issues : issues)],
      scores: { ...scoreMap },
      rows: scoreRows
    };
    ...
    if (resultBox) {
      ...
      const issueText = isLiveMode && !aiDiagnose
        ? "Live AI was called, but the response could not be parsed. Using fallback."
        : `Live issues: ${issueList.length ? issueList.join(", ") : "None"}`;
      await typeWriter(live, issueText, 12);
    }

    const fallback = generateLocalInsight(input, scoreMap);
    const insightText = aiDiagnose?.insight || fallback.insight;
    const repairText = aiDiagnose?.repair || fallback.repair;
...
async function generateRepair() {
  ...
  try {
    ...
    const fallbackRepair = ["Suggestions:", "", ...suggestions.map((suggestion) => `* ${suggestion}`)].join("\n");
    const repaired = isLiveMode && !aiRepair
      ? `Live AI was called, but the response could not be parsed. Using fallback.\n\n${fallbackRepair}`
      : (aiRepair?.repairedText || fallbackRepair);
